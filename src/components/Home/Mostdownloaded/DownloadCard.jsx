...
import FetchMostDownload from "../../../hooks/MostDownload";

const DownloadCard = ({ settings, indicators }) => {
  const reload = () => {
    window.location.reload();
  };
  const { data = [], isLoading, error } = FetchMostDownload();
  if (isLoading) {
    return <DataLoader />;
  }
  if (error) {
  ...
  if (!data || !data.data || !data.data.mostdownload) {
    return (
      <div>
        <div className="empty-pending-friends">
          <div className="card-profile-name d-flex justify-content-center flex-column align-items-center">
  ...
  const mostdownload = data.data.mostdownload;
  ...
  return (
    <div className="">
      {mostdownload.length !== 0 ? (
        <Slider className="custom-slider" {...settings}>
          {mostdownload.map((data, index) => (
